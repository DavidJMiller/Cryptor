...
void AES_encrypt(const uint8_t* input, const uint8_t* key, uint8_t* output, const uint32_t length)
{
...
void AES_decrypt(const uint8_t* input, const uint8_t* key, uint8_t *output, const uint32_t length)
{
  // Copy input to output, and work in-memory on output
  memcpy(output, input, length);
  state = (state_t*)output;
