...
int main(int argc, char **argv){
  ...
  const uint8_t nonce[8] = {0x15, 0x69, 0xdb, 0xfd, 0xa5, 0xea, 0x82, 0x24};
  uint8_t iv[16];
  ...
  while(fread(buffer, 1, 16, file_open)){
    ...
    /*for (i = 0; i < 16; ++i)
      printf("%x", ciphered[i]);
      printf("\n");*/
    size_t count_write = fwrite(buffer, 1, 16, out_open);
    if (count_write != 16){
      perror("Write error\n");
    }
    
    //increment counter part of iv
    ++count;

    //build iv for next round
    memset(iv, 1, sizeof(iv));
    snprintf(iv, sizeof(iv), "%s%d", nonce, count);
